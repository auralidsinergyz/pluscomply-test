			...
			$( '.parent_field select' ).on( 'change', function( e ) {
				...
				if ( el.attr( 'name' ).indexOf( 'course' ) != '-1' ) {
					if ( parent_id.length > 0 ) {
						lesson_field.select2( 'destroy' ).select2( LD_Notifications.build_select2_args( child_field_select2_args ) );
					} else {
				...
				if ( el.attr( 'name' ).indexOf( 'lesson' ) != '-1' ) {
					if ( parent_id.length > 0 ) {
						topic_field.select2( 'destroy' ).select2( LD_Notifications.build_select2_args( child_field_select2_args ) );
					} else {
						child_fields = [ topic_field, quiz_field ];
						LD_Notifications.reset_child_fields( child_fields );
					}
				}
				...
				if ( el.attr( 'name' ).indexOf( 'topic' ) != '-1' ) {
					if ( parent_id.length > 0 ) {
						quiz_field.select2( 'destroy' ).select2( LD_Notifications.build_select2_args( child_field_select2_args ) );
					} else {
						child_fields = [ quiz_field ];
						LD_Notifications.reset_child_fields( child_fields );
					}
				}
			});
		...
		reset_child_fields: function( fields = [] ) {
			fields.map( function( field ) {
				...
				if ( field.attr( 'name' ).indexOf( 'lesson_id' ) > -1 ) {
					...
					placeholder = LearnDash_Notifications_Vars.select_topic_first;
				}
		...
		init_select2_fields: function( parent = null ) {
			...
			selects.each( function( $el, index ) {
				const wrapper      = $( this ).closest( '.sfwd_input' ),
					...
					disabled_child = wrapper.hasClass( 'disabled-child' ) && value.length < 1 ? true : false,
					el             = $( this ),
					name           = $( this ).attr( 'name' ),
					inputs_wrapper = el.closest( '.inputs-wrapper' );
			...
			$( '#posts-filter select.select2' ).on( 'change', function( e ) {
				...
				if ( el.attr( 'name' ).indexOf( 'course' ) != '-1' ) {
					...
					$( '#posts-filter select[name="quiz_id"]' ).html(
						'<option value="">' + LearnDash_Notifications_Vars.select_topic_first + '</option>'
					).attr( 'disabled', true );
