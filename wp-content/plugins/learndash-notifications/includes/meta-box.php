...
function learndash_notifications_meta_box_output( $key, $args, $echo = false, $context = '', $condition_key = null ): string {
	...
	if ( 'delay_unit' === $key && empty( $key_value ) ) {
		...
		$key_value = intval( $key_value ) === 0 ? 'all' : $key_value;
	...
	if ( is_array( $key_value ) ) {
		...
		foreach ( $key_value as $value ) {
			if ( is_string( $value ) && strlen( $value ) && $value == 0 ) {
				...
				// Set pre-selected value if it's available in database.
			...
			<div class="sfwd_option_div">
				...
				<?php elseif ( $args['type'] === 'checkbox' ) : ?>
					...
					<?php else : ?>
                        ...
                        <input type="checkbox" name="<?php echo $cb_input_name; ?>" id="<?php echo $value; ?>"
                               value="<?php echo $value; ?>" <?php echo $checked; ?> <?php echo $disabled; ?>>
                        <label for="<?php echo $value; ?>"><?php echo $args['label']; ?></label><br/>
...
function learndash_notifications_get_shortcodes_instructions() {
	...
	$instructions = array(
		...
		'pass_quiz'            => array_merge( $user_shortcode, $course_basic_shortcode, $lesson_shortcode, $topic_shortcode, $quiz_shortcode ),
		'fail_quiz'            => array_merge( $user_shortcode, $course_basic_shortcode, $lesson_shortcode, $topic_shortcode, $quiz_shortcode ),
		'submit_quiz'          => array_merge( $user_shortcode, $course_basic_shortcode, $lesson_shortcode, $topic_shortcode, $quiz_shortcode ),
		'complete_quiz'        => array_merge( $user_shortcode, $course_basic_shortcode, $lesson_shortcode, $topic_shortcode, $quiz_shortcode ),
