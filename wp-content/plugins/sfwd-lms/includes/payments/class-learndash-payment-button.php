...
if ( ! class_exists( 'Learndash_Payment_Button' ) ) {
	...
	class Learndash_Payment_Button {
		...
		public function map(): string {
			...
			try {
				$product = Product::create_from_post( $this->post );
			} catch ( Exception $e ) {
				return '';
			}
			...
			if ( $product->is_price_type_free() ) {
				return $this->button_free();
			}

			if ( $product->is_price_type_paynow() || $product->is_price_type_subscribe() ) {
