...
/*!*********************************************************!*\
  !*** ./includes/blocks/src/commons/common-functions.js ***!
  \*********************************************************/
...
function wisdm_reports_change_block_visibility(elementSelector) {
  let visible = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  let parent_element = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
  ...
  if (typeof isSavingPost !== 'undefined') {
    ...
    wp.data.subscribe(() => {
      if (isSavingPost()) {
        ...
        if (!checked) {
          if ('publish' == wp.data.select('core/editor').getEditedPostAttribute('status') && wisdm_ld_reports_common_script_data.dashboard_page_id == wp.data.select('core/editor').getEditedPostAttribute('id') && (wisdm_ld_reports_common_script_data.visited_dashboard == false || wisdm_ld_reports_common_script_data.visited_dashboard == 'free')) {
    ...
    wp.data.subscribe(() => {
      if (isSavingPost()) {
        ...
        if (!checked2) {
          if ('publish' == wp.data.select('core/editor').getEditedPostAttribute('status') && wisdm_ld_reports_common_script_data.student_page_id == wp.data.select('core/editor').getEditedPostAttribute('id') && wisdm_ld_reports_common_script_data.visited_student_dashboard == false) {
            window.location.href = wp.data.select('core/editor').getEditedPostAttribute('link');
          }
          checked2 = true;
        }
